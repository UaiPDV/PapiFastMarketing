		...
		<div className="flex flex-col">
				...
				<div className="max-w-7xl mx-auto px-4">
					...
					<div className="flex flex-col sm:flex-row justify-center gap-4">
						<a
							...
							className="accent px-8 py-3 rounded-xl font-bold shadow-lg transition hover:scale-105"
						>
							{data.hero.cta_button}
						</a>
						<button
							onClick={scrollToFeatures}
							className="bg-white/10 px-8 py-3 rounded-xl font-bold hover:bg-white/15 transition hover:scale-105 border border-white/10"
						>
			...
			>
				<div className="max-w-7xl mx-auto px-4 text-center">
					...
					<div className="grid md:grid-cols-3 gap-8">
						{data.valueProposition.items.map(
							(item: any, idx: number) => (
								<div
									key={idx}
									className="glass p-6 rounded-2xl text-left hover:bg-white/5 transition border border-white/5"
								>
									<div
										className="w-12 h-12 bg-orange-500/10 text-orange-500 rounded-lg flex items-center justify-center mb-4"
			...
			<section id="features" className="py-16 border-t border-white/10">
				<div className="max-w-7xl mx-auto px-4 text-center">
					...
					<p className="opacity-70 mt-2 mb-12 max-w-2xl mx-auto">
					...
					<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 text-left">
						{data.features.items.map((item: any, idx: number) => (
							<div
								key={idx}
								className="flex gap-4 p-4 rounded-xl hover:bg-white/5 transition"
							>
								<div
									className="w-12 h-12 flex-shrink-0 bg-orange-500/10 text-orange-500 rounded-lg flex items-center justify-center"
									dangerouslySetInnerHTML={{
										__html: item.icon,
									}}
								/>
			...
			{data.segments && (
				...
				>
					<div className="max-w-7xl mx-auto px-4 text-center">
						<p className="text-orange-400 font-semibold uppercase tracking-wider text-sm mb-2">
							SEGMENTOS
						</p>
						...
						<div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
							{data.segments.items.map(
								(item: any, idx: number) => (
									<div
										key={idx}
										className="glass p-6 rounded-3xl hover:bg-white/5 transition group"
									>
			...
			>
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
					<div className="text-center mb-12">
						...
						<p className="opacity-70 mt-2 max-w-3xl mx-auto text-lg">
							...
							{data.flows.map((flow: any, idx: number) => (
								<div key={idx} className="w-full px-4">
									<div className="text-center mb-10">
										...
										<p className="opacity-70 max-w-2xl mx-auto mt-2 text-lg">
						...
						<Carousel slidesToShow={3}>
							{data.screenshots.items.map(
								(item: any, idx: number) => (
									<div
										key={idx}
										className="glass rounded-2xl overflow-hidden group border border-white/5 h-full"
									>
					...
					<div className="glass rounded-3xl p-12 text-center md:text-left flex flex-col md:flex-row items-center justify-between gap-8 border border-white/10 bg-gradient-to-br from-white/5 to-transparent">
						<div>
							<p className="text-orange-400 font-semibold tracking-wide uppercase text-sm">
								{data.forClients.tagline}
							</p>
							<h2 className="text-3xl font-bold mt-2">
								{data.forClients.title}
							</h2>
							<p className="mt-4 opacity-80 text-lg">
								{data.forClients.description}
							</p>
						</div>
						<a
							href={whatsappLink}
							target="_blank"
							rel="noopener noreferrer"
							className="accent px-8 py-3 rounded-xl font-bold shadow-lg whitespace-nowrap transition hover:scale-105"
						>
				...
				<div className="max-w-4xl mx-auto px-4">
					...
					<div className="flex justify-center">
						<a
							...
							target="_blank"
							rel="noopener noreferrer"
							className="accent px-8 py-4 rounded-xl font-bold shadow-lg transition hover:scale-105 text-lg"
						>
							{data.cta.button}
						</a>
					</div>
