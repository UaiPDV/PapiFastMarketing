...
export const Business: React.FC = () => {
  ...
  return (
    <div className="flex flex-col">
               ...
               {data.infoSections.map((section: any, idx: number) => (
                   <div key={idx} className={`flex flex-col md:flex-row gap-12 items-center ${idx % 2 !== 0 ? 'md:flex-row-reverse' : ''} reveal`}>
                       ...
                       <div className="flex-1 w-full">
                           <img src={section.image} alt={section.title} className="rounded-2xl shadow-2xl w-full object-cover border border-white/10" />
                       </div>
                   </div>
               ))}
           </div>
       ...
       {/* Segments (NEW) */}
       {data.segments && (
           <section id="segments" className="py-16 border-t border-white/10">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <p className="text-orange-400 font-semibold uppercase tracking-wider text-sm mb-2">SEGMENTOS</p>
                    <h2 className="text-3xl md:text-4xl font-bold mb-12">{data.segments.title}</h2>
                    
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {data.segments.items.map((item: any, idx: number) => (
