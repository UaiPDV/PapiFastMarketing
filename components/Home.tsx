...
export const Home: React.FC = () => {
  ...
  const data = content.homePage;
    ...
    const getImageSrc = (src?: string) => resolveAssetPath(src) || src || 'https://placehold.co/600x400/1f2937/ffffff?text=Preview';
    ...
    <div className="flex flex-col">
      ...
      <section id="sobre" className="py-16 border-t border-white/10">
        <div className="max-w-3xl mx-auto px-4 text-center reveal">
            <h2 className="text-2xl md:text-3xl font-bold">{data.about.title}</h2>
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-8 bg-white/5 p-1.5 rounded-xl inline-flex">
                  {data.businessCta.tabs.map((tab: any, idx: number) => (
                      <button 
                        ...
                        className={`px-6 py-2.5 rounded-lg font-medium transition-all duration-300 ${activeTab === idx ? 'accent shadow-lg text-white' : 'hover:bg-white/5 text-white/70'}`}
                      >
                          {tab.title}
                      </button>
                  ))}
