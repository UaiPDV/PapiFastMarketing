...
export const Admin: React.FC = () => {
		...
		const findAndScroll = (attempt = 1) => {
			...
			if (element) {
				...
				if (attempt < 15) {
					...
					console.warn(
						'Element not found for path after retries:',
						result.path
					);
				}
			}
	...
	if (loading || !formData)
		return (
			<div className="min-h-screen flex items-center justify-center bg-[#0f0f11] text-white pt-16">
				Carregando editor...
			</div>
		);
		...
		return (
			<div className="mb-4">
				...
				{textarea ? (
					<textarea
						data-path={path}
						className={`w-full border rounded-lg p-3 outline-none text-white h-24 resize-y transition-all duration-300 ${
							isActive
								? 'bg-orange-900/20 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)]'
								: 'bg-gray-900/50 border-gray-700 focus:border-orange-500 focus:bg-gray-900'
						}`}
						value={value || ''}
					...
					<input
						data-path={path}
						type="text"
						className={`w-full border rounded-lg p-3 outline-none text-white transition-all duration-300 ${
							isActive
								? 'bg-orange-900/20 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)]'
								: 'bg-gray-900/50 border-gray-700 focus:border-orange-500 focus:bg-gray-900'
						}`}
	...
	const SectionCard = ({
		...
		return (
			<div
				id={id}
				className="bg-gray-800/50 border border-gray-700/50 rounded-xl overflow-hidden mb-8 shadow-sm transition-all duration-300"
			>
				<div
					className={`p-6 border-b border-gray-700/50 bg-gray-800 flex justify-between items-center cursor-pointer hover:bg-gray-750 transition-colors ${
			...
			>
				...
				<nav className="flex-1 overflow-y-auto py-6 space-y-1 custom-scrollbar">
					...
					<div className="px-4 mb-6 space-y-2">
						<Link
							to="/business"
							className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-800/50 hover:bg-gray-800 border border-transparent hover:border-orange-500/30 transition-all text-sm font-medium"
						>
					...
					<div className="mt-4 px-4">
						<button
							...
							className="flex items-center gap-3 w-full px-4 py-3 rounded-xl hover:bg-white/5 transition-all text-sm font-medium text-gray-400 hover:text-white"
						>
							<Menu size={18} />
							Rodapé Global
						</button>
							...
							<div className="mt-6 grid gap-4">
								{formData.homePage.features.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 bg-gray-900 rounded-lg border border-gray-700 relative group"
										>
											<button
												onClick={() =>
													removeItem(
							...
							<div className="grid gap-4 mt-6">
								{formData.homePage.screenshots.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="flex gap-4 items-center p-3 bg-gray-900 rounded-lg border border-gray-700 group"
										>
							...
							<div className="space-y-8">
								{formData.businessPage.infoSections.map(
									(section: any, idx: number) => (
										<div
											key={idx}
											className="p-6 bg-gray-900 rounded-xl border border-gray-700 relative"
										>
											<button
												onClick={() =>
													removeItem(
							...
							<div className="grid gap-4 mt-6">
								{formData.businessPage.features.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 bg-gray-900 rounded-lg border border-gray-700 relative"
										>
											<button
												onClick={() =>
													removeItem(
						...
						<div className="space-y-4">
							{formData.homePage.footer.links.map(
								(link: any, idx: number) => (
									<div
										key={idx}
										className="flex gap-4 items-center p-3 bg-gray-900 rounded-lg border border-gray-700"
									>
										<div className="flex-grow space-y-2">
											<input
