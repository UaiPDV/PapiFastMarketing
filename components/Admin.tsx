...
const getSectionIdFromPath = (path: string): string => {
	...
	if (path.startsWith('businessPage.valueProposition'))
	...
	if (path.startsWith('businessPage.screenshots'))
		return 'business-screenshots';
	if (path.startsWith('businessPage.testimonials'))
	...
	return (
		<div className="mb-4">
			...
			{textarea ? (
				<textarea
					data-path={path}
					className={`w-full rounded-lg p-3 outline-none h-24 resize-y transition-all duration-300 admin-input ${
						isActive
				...
				<input
					data-path={path}
					type="text"
					className={`w-full rounded-lg p-3 outline-none transition-all duration-300 admin-input ${
						isActive
							? 'bg-orange-900/20 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)]'
							: 'focus:border-orange-500'
					}`}
		...
		if (file) {
			try {
				if (file.size > 800 * 1024) {
					if (
						!window.confirm(
							'Esta imagem é grande (>800KB) e pode demorar para salvar ou carregar. Deseja continuar? Recomenda-se usar imagens otimizadas ou links externos.'
						)
					) {
						return;
					}
	...
	return (
		<div className="w-full">
			...
			<div className="flex flex-col gap-3">
				<div
					className={`relative w-full h-32 admin-input rounded-lg flex items-center justify-center overflow-hidden group transition-all duration-300 ${
						isActive
				...
				<div className="flex gap-2">
					<div className="relative flex-grow">
						...
						<input
							data-path={path}
							type="text"
							className={`w-full admin-input rounded-lg pl-10 pr-3 py-2 text-sm focus:border-orange-500 outline-none transition-all duration-300 ${
...
const SectionCard = ({
	...
	return (
		<div
			id={id}
			className="admin-card rounded-xl overflow-hidden mb-8 shadow-sm transition-all duration-300"
		>
			<div
				...
				<h3 className="text-xl font-bold flex items-center gap-2">
					<span
						className={`w-1 h-6 rounded-full transition-colors ${
							isOpen ? 'bg-orange-500' : 'bg-gray-600'
						}`}
					></span>
					{title}
				</h3>
			...
			{isOpen && <div className="p-6 animate-fadeIn">{children}</div>}
		</div>
	);
};
...
export const Admin: React.FC = () => {
	...
	useEffect(() => {
		...
		const traverse = (obj: any, currentPath: string) => {
			if (typeof obj === 'string' || typeof obj === 'number') {
				...
				if (valStr.startsWith('data:image') && valStr.length > 100)
					return;
	...
	if (loading || !formData)
		return (
			<div className="min-h-screen flex items-center justify-center admin-wrapper pt-16">
				Carregando editor...
			</div>
		);
	...
	const aboutImages =
		formData.homePage.about.images ||
		(formData.homePage.about.image ? [formData.homePage.about.image] : []);
			...
			>
				...
				<nav className="flex-1 overflow-y-auto py-6 space-y-1 custom-scrollbar">
					...
					<div className="px-4 mb-6 space-y-2">
						<Link
							to="/business"
							className="flex items-center gap-3 px-4 py-3 rounded-xl bg-black/5 hover:bg-black/10 border border-transparent hover:border-orange-500/30 transition-all text-sm font-medium text-gray-500 hover:text-orange-500"
						>
					...
					<div className="mt-4 px-4">
						<button
							onClick={() => expandSection('site-footer')}
							className="flex items-center gap-3 w-full px-4 py-3 rounded-xl hover:bg-black/5 transition-all text-sm font-medium text-gray-400 hover:text-orange-500"
						>
							<Menu size={18} />
							Rodapé Global
						</button>
							...
							{results.length > 0 && (
								<div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 bg-gray-700/50 rounded p-1">
									...
									<button
										...
										className="hover:text-orange-400 text-gray-400 p-0.5 rounded hover:bg-white/10"
				...
				<div className="p-6 lg:p-10 max-w-5xl mx-auto space-y-6">
					...
					<div className="mb-12">
						...
						<SectionCard
							title="Hero (Topo)"
							id="home-hero"
							isOpen={!!expandedSections['home-hero']}
							onToggle={() => toggleSection('home-hero')}
						>
							<Input
								label="Título (Parte 1)"
								path="homePage.hero.title_part1"
								value={getNestedValue(
									formData,
									'homePage.hero.title_part1'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
									'homePage.hero.title_part1'
								}
							/>
							<Input
								label="Título (Destaque)"
								path="homePage.hero.title_highlight"
								value={getNestedValue(
									formData,
									'homePage.hero.title_highlight'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid grid-cols-2 gap-4">
								<Input
									...
									path="homePage.hero.primary_button"
									value={getNestedValue(
										formData,
										'homePage.hero.primary_button'
									)}
									onChange={handleChange}
									isActive={
										results[currentResultIndex]?.path ===
								...
								<Input
									...
									path="homePage.hero.secondary_button"
									value={getNestedValue(
										formData,
										'homePage.hero.secondary_button'
									)}
									onChange={handleChange}
									isActive={
										results[currentResultIndex]?.path ===
							...
							<Input
								...
								path="homePage.hero.primary_button_link"
								value={getNestedValue(
									formData,
									'homePage.hero.primary_button_link'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="mt-4 pt-4 border-t border-gray-700">
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									<ImageInput
										...
										value={
											formData.homePage.hero.image_left ||
											''
										}
										onChange={(val) =>
											handleChange(
												'homePage.hero.image_left',
												val
											)
										}
										isActive={
									...
									<ImageInput
										...
										value={
											formData.homePage.hero
												.image_center || ''
										}
										onChange={(val) =>
											handleChange(
												'homePage.hero.image_center',
												val
											)
										}
										isActive={
						...
						<SectionCard
							title="Sobre"
							id="home-about"
							isOpen={!!expandedSections['home-about']}
							onToggle={() => toggleSection('home-about')}
						>
							<Input
								label="Título"
							...
							<div className="mt-6">
								...
								<p className="text-xs text-gray-500 mb-4">
									Adicione até 4 imagens para o efeito de
									"fotos jogadas".
								</p>
								...
								<div className="space-y-6">
									{aboutImages.map(
										(img: string, idx: number) => (
											<div
												key={idx}
												className="p-4 admin-input rounded-lg relative"
											>
												<button
													onClick={() => {
														const newImages = [
						...
						<SectionCard
							title="Como Funciona"
							id="home-how"
							isOpen={!!expandedSections['home-how']}
							onToggle={() => toggleSection('home-how')}
						>
							<Input
								label="Título da Seção"
								path="homePage.howItWorks.title"
								value={getNestedValue(
									formData,
							...
							<div className="mb-4">
								<ImageInput
									...
									value={
										formData.homePage.howItWorks.image || ''
									}
									onChange={(val) =>
										handleChange(
											'homePage.howItWorks.image',
											val
										)
									}
									isActive={
							...
							<div className="space-y-4 mt-6">
								{formData.homePage.howItWorks.steps.map(
									(step: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg"
										>
						...
						<SectionCard
							...
							id="home-features"
							isOpen={!!expandedSections['home-features']}
							onToggle={() => toggleSection('home-features')}
						>
							<Input
								label="Título"
							...
							<div className="mt-6 grid gap-4">
								{formData.homePage.features.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg relative group"
										>
											<button
												onClick={() =>
													removeItem(
								...
								<button
									onClick={() =>
										addItem('homePage.features.items', {
						...
						<SectionCard
							title="Casos de Uso"
							id="home-usecases"
							isOpen={!!expandedSections['home-usecases']}
							onToggle={() => toggleSection('home-usecases')}
						>
							<Input
								label="Título"
								path="homePage.useCases.title"
								value={getNestedValue(
									formData,
							...
							<Input
								label="Subtítulo"
								path="homePage.useCases.subtitle"
								value={getNestedValue(
									formData,
									'homePage.useCases.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid gap-4 mt-6">
								{(formData.homePage.useCases?.items || []).map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg relative"
											...
											<div className="mb-4">
												<ImageInput
													...
													isActive={
														...
														`homePage.useCases.items.${idx}.image`
													}
												/>
											</div>
								...
								<button
									onClick={() => {
										if (!formData.homePage.useCases) {
											...
											addItem('homePage.useCases.items', {
												title: 'Novo Caso',
												description: '',
												image: '',
											});
						...
						<SectionCard
							title="Fluxos (Diagramas)"
							id="home-flows"
							isOpen={!!expandedSections['home-flows']}
							onToggle={() => toggleSection('home-flows')}
						>
							<Input
								label="Título Geral"
								path="homePage.flowsConfig.title"
								value={getNestedValue(
									formData,
									'homePage.flowsConfig.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								...
								path="homePage.flowsConfig.subtitle"
								textarea
								value={getNestedValue(
									formData,
									'homePage.flowsConfig.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							{formData.homePage.flows.map(
								(flow: any, fIdx: number) => (
									<div
										key={fIdx}
										className="mt-8 p-6 border border-orange-500/20 rounded-xl bg-orange-500/5 relative"
									>
										<button
											onClick={() =>
												removeItem(
							...
							<button
								onClick={() =>
									addItem('homePage.flows', {
										sectionTitle: 'Novo Fluxo',
										sectionSubtitle: '',
										steps: [{}, {}, {}, {}],
									})
								}
						...
						<SectionCard
							title="Telas (Screenshots)"
							id="home-screenshots"
							isOpen={!!expandedSections['home-screenshots']}
							onToggle={() => toggleSection('home-screenshots')}
						>
							<Input
								label="Título"
								path="homePage.screenshots.title"
								value={getNestedValue(
									formData,
									'homePage.screenshots.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="homePage.screenshots.subtitle"
								value={getNestedValue(
									formData,
									'homePage.screenshots.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid gap-4 mt-6">
								{formData.homePage.screenshots.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="flex gap-4 items-start p-4 admin-input rounded-lg group"
										>
											<div className="flex-grow space-y-4">
												<ImageInput
													...
													isActive={
														...
														`homePage.screenshots.items.${idx}.image`
													}
												/>
												<input
								...
								<button
									onClick={() =>
										addItem('homePage.screenshots.items', {
											image: '',
											caption: '',
										})
									}
									className="text-sm bg-gray-800 hover:bg-gray-700 py-2 rounded text-white transition"
								>
									+ Adicionar Imagem
								</button>
							</div>
						...
						<SectionCard
							title="Depoimentos"
							id="home-testimonials"
							isOpen={!!expandedSections['home-testimonials']}
							onToggle={() => toggleSection('home-testimonials')}
						>
							<Input
								label="Título"
							...
							<div className="space-y-4 mt-6">
								{formData.homePage.testimonials.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg relative group"
								...
								<button
									onClick={() =>
										addItem('homePage.testimonials.items', {
											quote: '',
											author: '',
										})
									}
									className="text-sm bg-gray-800 hover:bg-gray-700 py-2 rounded text-white w-full transition"
								>
						...
						<SectionCard
							title="Chamada para Empresas"
							id="home-business"
							isOpen={!!expandedSections['home-business']}
							onToggle={() => toggleSection('home-business')}
						>
							<Input
								label="Tagline"
								path="homePage.businessCta.tagline"
								value={getNestedValue(
									formData,
									'homePage.businessCta.tagline'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Título"
								path="homePage.businessCta.title"
								value={getNestedValue(
									formData,
									'homePage.businessCta.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="homePage.businessCta.subtitle"
								textarea
								value={getNestedValue(
									formData,
									'homePage.businessCta.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Botão"
								path="homePage.businessCta.button"
								value={getNestedValue(
									formData,
									'homePage.businessCta.button'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="mt-6">
								<h4 className="text-gray-400 text-sm font-bold uppercase tracking-wider mb-3">
									Abas de Conteúdo
								</h4>
								<div className="space-y-4">
									{formData.homePage.businessCta.tabs.map(
										(tab: any, idx: number) => (
											<div
												key={idx}
												className="p-4 admin-input rounded-lg"
											>
												<div className="text-xs text-orange-400 font-bold mb-3 uppercase">
													Aba {idx + 1}
												</div>
						...
						<SectionCard
							title="Contato"
							id="home-contact"
							isOpen={!!expandedSections['home-contact']}
							onToggle={() => toggleSection('home-contact')}
						>
							<Input
								label="Título"
					...
					<div className="mb-12">
						...
						<SectionCard
							title="Hero (Topo)"
							id="business-hero"
							isOpen={!!expandedSections['business-hero']}
							onToggle={() => toggleSection('business-hero')}
						>
							<Input
								label="Título Hero"
								path="businessPage.hero.title"
								value={getNestedValue(
									formData,
									'businessPage.hero.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								...
								path="businessPage.hero.subtitle"
								textarea
								value={getNestedValue(
									formData,
									'businessPage.hero.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid grid-cols-2 gap-4">
								<Input
									label="Botão Primário"
									path="businessPage.hero.cta_button"
									value={getNestedValue(
										formData,
										'businessPage.hero.cta_button'
									)}
									onChange={handleChange}
									isActive={
										results[currentResultIndex]?.path ===
								...
								<Input
									label="Botão Secundário"
									path="businessPage.hero.secondary_button"
									value={getNestedValue(
										formData,
										'businessPage.hero.secondary_button'
									)}
									onChange={handleChange}
									isActive={
										results[currentResultIndex]?.path ===
						...
						<SectionCard
							...
							id="business-value"
							isOpen={!!expandedSections['business-value']}
							onToggle={() => toggleSection('business-value')}
						>
							<Input
								label="Título da Seção"
								path="businessPage.valueProposition.title"
								value={getNestedValue(
									formData,
									'businessPage.valueProposition.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							/>
							<div className="grid gap-4 mt-6">
								{formData.businessPage.valueProposition.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg"
										>
											<div className="flex gap-4">
												<div
													className="w-10 h-10 bg-orange-500/10 rounded flex items-center justify-center text-orange-500 font-mono text-xs overflow-hidden"
													title="Ícone SVG"
												>
													SVG
												</div>
						...
						<SectionCard
							title="Detalhes (Info Sections)"
							id="business-info"
							isOpen={!!expandedSections['business-info']}
							onToggle={() => toggleSection('business-info')}
						>
							<div className="space-y-8">
								{formData.businessPage.infoSections.map(
									(section: any, idx: number) => (
										<div
											key={idx}
											className="p-6 admin-input rounded-xl relative"
										>
											<button
												onClick={() =>
													removeItem(
											...
											<h4 className="text-orange-400 font-bold mb-4 uppercase tracking-wider text-sm">
												Seção {idx + 1}
											</h4>
											...
											<div className="mb-4">
												<ImageInput
													...
													isActive={
														...
														`businessPage.infoSections.${idx}.image`
													}
												/>
											</div>

											<label className="block text-xs font-bold text-gray-500 mb-1">
												Pontos da Lista (um por linha):
											</label>
											<textarea
								...
								<button
									onClick={() =>
										addItem('businessPage.infoSections', {
											title: 'Novo Título',
											description: '',
											points: [],
											image: '',
								...
								>
									<Plus size={20} /> Adicionar Seção de
									Informação
								</button>
							</div>
						</SectionCard>
						...
						<SectionCard
							title="Funcionalidades"
							id="business-features"
							isOpen={!!expandedSections['business-features']}
							onToggle={() => toggleSection('business-features')}
						>
							<Input
								label="Título"
								path="businessPage.features.title"
								value={getNestedValue(
									formData,
									'businessPage.features.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="businessPage.features.subtitle"
								value={getNestedValue(
									formData,
									'businessPage.features.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid gap-4 mt-6">
								{formData.businessPage.features.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg relative"
								...
								<button
									onClick={() =>
										addItem('businessPage.features.items', {
											title: 'Nova',
											description: '',
											icon: '<svg>...</svg>',
										})
									...
									className="w-full py-3 border-2 border-dashed border-gray-700 rounded-lg text-gray-400 hover:text-orange-500 transition flex justify-center gap-2 items-center"
								>
									<Plus size={20} /> Adicionar Feature
								</button>
							</div>
						</SectionCard>
						...
						<SectionCard
							title="Segmentos"
							id="business-segments"
							isOpen={!!expandedSections['business-segments']}
							onToggle={() => toggleSection('business-segments')}
						>
							<Input
								label="Título"
								path="businessPage.segments.title"
								value={getNestedValue(
									formData,
									'businessPage.segments.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="businessPage.segments.subtitle"
								value={getNestedValue(
									formData,
									'businessPage.segments.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid gap-4 mt-6">
								{(
									formData.businessPage.segments?.items || []
										...
										<div className="mb-4">
											<ImageInput
												...
												isActive={
													results[currentResultIndex]
														?.path ===
													`businessPage.segments.items.${idx}.image`
												}
											/>
										</div>
						...
						<SectionCard
							title="Fluxos (Business)"
							id="business-flows"
							isOpen={!!expandedSections['business-flows']}
							onToggle={() => toggleSection('business-flows')}
						>
							<Input
								label="Título Geral"
								path="businessPage.flowsConfig.title"
								value={getNestedValue(
									formData,
									'businessPage.flowsConfig.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo Geral"
								path="businessPage.flowsConfig.subtitle"
								textarea
								value={getNestedValue(
									formData,
									'businessPage.flowsConfig.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							{formData.businessPage.flows.map(
								(flow: any, fIdx: number) => (
									<div
										key={fIdx}
										className="mt-8 p-6 border border-orange-500/20 rounded-xl bg-orange-500/5 relative"
						...
						<SectionCard
							title="Telas (Screenshots)"
							id="business-screenshots"
							isOpen={!!expandedSections['business-screenshots']}
							onToggle={() =>
								toggleSection('business-screenshots')
							}
						>
							<Input
								label="Título"
								path="businessPage.screenshots.title"
								value={getNestedValue(
									formData,
									'businessPage.screenshots.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="businessPage.screenshots.subtitle"
								value={getNestedValue(
									formData,
									'businessPage.screenshots.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<div className="grid gap-4 mt-6">
								{formData.businessPage.screenshots.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="flex gap-4 items-start p-4 admin-input rounded-lg group"
											...
											<div className="flex-grow space-y-4">
												<ImageInput
													...
													isActive={
														...
														`businessPage.screenshots.items.${idx}.image`
													}
												/>
												<input
						...
						<SectionCard
							title="Depoimentos"
							id="business-testimonials"
							isOpen={!!expandedSections['business-testimonials']}
							onToggle={() =>
								toggleSection('business-testimonials')
							}
						>
							<Input
								label="Título"
								path="businessPage.testimonials.title"
								value={getNestedValue(
									formData,
									'businessPage.testimonials.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Subtítulo"
								path="businessPage.testimonials.subtitle"
								value={getNestedValue(
									formData,
									'businessPage.testimonials.subtitle'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							/>
							<div className="space-y-4 mt-6">
								{formData.businessPage.testimonials.items.map(
									(item: any, idx: number) => (
										<div
											key={idx}
											className="p-4 admin-input rounded-lg relative group"
						...
						<SectionCard
							title="Chamada para Clientes"
							id="business-clients"
							isOpen={!!expandedSections['business-clients']}
							onToggle={() => toggleSection('business-clients')}
						>
							<Input
								label="Tagline"
								path="businessPage.forClients.tagline"
								value={getNestedValue(
									formData,
									'businessPage.forClients.tagline'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Título"
								path="businessPage.forClients.title"
								value={getNestedValue(
									formData,
									'businessPage.forClients.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Descrição"
								path="businessPage.forClients.description"
								textarea
								value={getNestedValue(
									formData,
									'businessPage.forClients.description'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Botão"
								path="businessPage.forClients.button"
								value={getNestedValue(
									formData,
									'businessPage.forClients.button'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
						...
						<SectionCard
							title="CTA Final"
							id="business-cta"
							isOpen={!!expandedSections['business-cta']}
							onToggle={() => toggleSection('business-cta')}
						>
							<Input
								label="Título"
								path="businessPage.cta.title"
								value={getNestedValue(
									formData,
									'businessPage.cta.title'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Descrição"
								path="businessPage.cta.description"
								textarea
								value={getNestedValue(
									formData,
									'businessPage.cta.description'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
							...
							<Input
								label="Botão"
								path="businessPage.cta.button"
								value={getNestedValue(
									formData,
									'businessPage.cta.button'
								)}
								onChange={handleChange}
								isActive={
									results[currentResultIndex]?.path ===
					...
					<SectionCard
						title="Rodapé (Global)"
						id="site-footer"
						isOpen={!!expandedSections['site-footer']}
						onToggle={() => toggleSection('site-footer')}
					>
						<div className="space-y-4">
							{formData.homePage.footer.links.map(
								(link: any, idx: number) => (
									<div
										key={idx}
										className="flex gap-4 items-center p-3 admin-input rounded-lg"
									>
